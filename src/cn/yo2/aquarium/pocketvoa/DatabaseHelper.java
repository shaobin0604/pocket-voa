	...
	public Article queryArticle(long id) {
		...
		if (cursor != null) {
			if (cursor.moveToFirst()) {
				...
				article.urltext = cursor.getString(cursor.getColumnIndex(C_URLTEXT));
				article.urlmp3 = cursor.getString(cursor.getColumnIndex(C_URLMP3));
				
				article.urltextzh = cursor.getString(cursor.getColumnIndex(C_URLTEXTZH));
				article.urllrc = cursor.getString(cursor.getColumnIndex(C_URLLRC));
				article.hastextzh = cursor.getInt(cursor.getColumnIndex(C_HASTEXTZH)) == 1;
	...
	private static class DatabaseOpenHelper extends SQLiteOpenHelper {
		...
		public void onUpgrade(SQLiteDatabase db, int oldVersion, int newVersion) {
			...
			if (oldVersion == 3 && newVersion == 4) {
				...
				db.beginTransaction();
				try {
					db.execSQL("CREATE TEMPORARY TABLE " + T_ARTICLES
							+ "_backup("
							+ "_id INTEGER PRIMARY KEY AUTOINCREMENT,"
							+ "type TEXT," + "subtype TEXT," + "title TEXT,"
							+ "date INTEGER," + "url TEXT," + "mp3 TEXT"
							+ ");");
